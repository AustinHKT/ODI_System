...
async function ldR() { try { const r=await window.storage.get("odi-records"); return r?JSON.parse(r.value):[]; } catch{return[];} }
async function svR(r) { try { await window.storage.set("odi-records",JSON.stringify(r)); } catch(e){console.error(e);} }
async function ldU() { try { const r=await window.storage.get("odi-users"); return r?JSON.parse(r.value):[{id:"admin",username:"admin",password:"1234",displayName:"ผู้ดูแลระบบ",role:"admin",createdAt:new Date().toISOString()}]; } catch{return [{id:"admin",username:"admin",password:"1234",displayName:"ผู้ดูแลระบบ",role:"admin",createdAt:new Date().toISOString()}];} }
async function svU(u) { try { await window.storage.set("odi-users",JSON.stringify(u)); } catch(e){console.error(e);} }
...
function ResultPage({patient,answers,skippedSections,onNewPatient}) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{const recs=await ldR();recs.unshift(record);await svR(recs);console.log("✅ ODI record saved:",record.id);}catch(e){console.error("❌ Save failed:",e);}
    })();
  },[]);
...
function AdminLogin({onLogin,onBack}) {
  ...
  const resetPw=async()=>{
    ...
    const updated=users.map(x=>x.id===found.id?{...x,password:rNewPw}:x);
    await svU(updated);
    sMode("resetDone");
  };
...
function AdminPanel({currentUser,onLogout}) {
  ...
  useEffect(()=>{
    const handleStorage=(e)=>{
      if(e.key==="odi-records"||e.key==="odi-users"){refreshData();}
    };
      ...
      <div style={{flex:1,overflow:"auto"}}>
        ...
        <div style={{padding:"24px 28px"}}>
          {tab==="dashboard"&&(<>
            ...
            {/* Search */}
            <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}><input value={search} onChange={e=>sSch(e.target.value)} placeholder="🔍 ค้นหาชื่อหรือเบอร์โทร..." style={{flex:1,minWidth:200,padding:"10px 14px",border:"2px solid #e2e8f0",borderRadius:10,fontSize:14,outline:"none",background:"#fff"}}/><select value={fl} onChange={e=>sFl(e.target.value)} style={{padding:"10px 14px",border:"2px solid #e2e8f0",borderRadius:10,fontSize:14,background:"#fff",outline:"none"}}><option value="all">ทุกระดับ</option><option value="Minimal Disability">Minimal (0-20%)</option><option value="Moderate Disability">Moderate (21-40%)</option><option value="Severe Disability">Severe (41-60%)</option><option value="Crippling Disability">Crippling (61-80%)</option><option value="Bed-bound or Exaggerated Disability">Bed-bound (81-100%)</option></select></div>

...
async function ldR() { try { const r=localStorage.getItem("odi-records"); return r?JSON.parse(r):[]; } catch{return[];} }
async function svR(r) { try { localStorage.setItem("odi-records",JSON.stringify(r)); } catch(e){console.error(e);} }
async function ldU() { try { const r=localStorage.getItem("odi-users"); return r?JSON.parse(r):[{id:"admin",username:"admin",password:"1234",displayName:"ผู้ดูแลระบบ",role:"admin",createdAt:new Date().toISOString()}]; } catch{return [{id:"admin",username:"admin",password:"1234",displayName:"ผู้ดูแลระบบ",role:"admin",createdAt:new Date().toISOString()}];} }
async function svU(u) { try { localStorage.setItem("odi-users",JSON.stringify(u)); } catch(e){console.error(e);} }
      ...
      <div style={{width:"100%",maxWidth:520}}>
        <div style={{background:"#fff",borderRadius:20,boxShadow:"0 8px 32px rgba(30,58,92,0.10)",overflow:"hidden"}}>
          ...
          <div style={{padding:"28px 32px 36px"}}>
            ...
            <div style={{marginBottom:24}}><label style={{display:"block",fontSize:13,fontWeight:600,color:"#475569",marginBottom:6}}>HN (Hospital Number) <span style={{fontWeight:400,color:"#94a3b8"}}>— ไม่บังคับ</span></label><input value={hn} onChange={e=>sHn(e.target.value)} placeholder="เช่น HN-001234" style={IS} onFocus={fi} onBlur={fo}/></div>
...
function ResultPage({patient,answers,skippedSections,onNewPatient}) {
  ...
  const [cap,sCap]=useState(false),[saved,sSaved]=useState(false);
  useEffect(()=>{if(saved)return;(async()=>{const r={id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),patient,answers,skippedSections,totalScore:ts,maxScore:ms,percentage:pct,level:res.level,levelTH:res.levelTH,submittedAt:new Date().toISOString()};const recs=await ldR();recs.unshift(r);await svR(recs);sSaved(true);})();},[]);
      ...
      <div style={{width:"100%",maxWidth:600,paddingTop:20}} ref={ref}>
        <div style={{background:"#fff",borderRadius:20,boxShadow:"0 8px 32px rgba(30,58,92,0.10)",overflow:"hidden"}}>
          <div style={{background:"linear-gradient(135deg, #1e3a5c 0%, #2563eb 100%)",padding:"28px 32px",color:"#fff",textAlign:"center"}}>
            ...
            {patient.hn&&<div style={{fontSize:13,opacity:0.7,marginTop:4}}>HN: {patient.hn}</div>}
          </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg, #0f172a 0%, #1e3a5c 60%, #1e40af 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",fontFamily:FONT}}>
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{textAlign:"center",marginBottom:32}}><div style={{width:64,height:64,background:"rgba(255,255,255,0.1)",borderRadius:16,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:16,border:"1px solid rgba(255,255,255,0.15)"}}>🔐</div><div style={{color:"#fff",fontSize:22,fontWeight:700}}>ระบบหลังบ้าน ODI</div><div style={{color:"rgba(255,255,255,0.6)",fontSize:14,marginTop:6}}>เข้าสู่ระบบสำหรับเจ้าหน้าที่</div></div>
        <div style={{background:"rgba(255,255,255,0.05)",borderRadius:20,padding:"32px",border:"1px solid rgba(255,255,255,0.1)"}}>
          ...
          <div style={{marginBottom:18}}><label style={{display:"block",fontSize:13,fontWeight:600,color:"rgba(255,255,255,0.7)",marginBottom:8}}>ชื่อผู้ใช้</label><input value={u} onChange={e=>sU(e.target.value)} placeholder="username" onKeyDown={e=>e.key==="Enter"&&go()} style={{width:"100%",padding:"14px 16px",background:"rgba(255,255,255,0.08)",border:"2px solid rgba(255,255,255,0.15)",borderRadius:12,fontSize:15,color:"#fff",outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="#3b82f6"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.15)"}/></div>
          <div style={{marginBottom:24}}><label style={{display:"block",fontSize:13,fontWeight:600,color:"rgba(255,255,255,0.7)",marginBottom:8}}>รหัสผ่าน</label><input value={p} onChange={e=>sP(e.target.value)} type="password" placeholder="••••••" onKeyDown={e=>e.key==="Enter"&&go()} style={{width:"100%",padding:"14px 16px",background:"rgba(255,255,255,0.08)",border:"2px solid rgba(255,255,255,0.15)",borderRadius:12,fontSize:15,color:"#fff",outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="#3b82f6"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.15)"}/></div>
...
function AdminPanel({currentUser,onLogout}) {
  ...
  const [expandCard,sEC]=useState(null); // which stat card is expanded
  const [exporting,sExp]=useState(false);
  useEffect(()=>{Promise.all([ldR(),ldU()]).then(([r,u])=>{sRec(r);sUs(u);sLd(false);});},[]);
  const filt=records.filter(r=>{const ms=r.patient.name.toLowerCase().includes(search.toLowerCase())||(r.patient.hn||"").toLowerCase().includes(search.toLowerCase());const ml=fl==="all"||r.level===fl;return ms&&ml;});
  ...
  const exportExcel = async () => {
    sExp(true);
    try {
      const header = ["ลำดับ","ชื่อ-สกุล","HN","วันเกิด","อายุ","เพศ","ODI Score (%)","คะแนนรวม","คะแนนเต็ม","ระดับ","ระดับ (EN)","วันที่ประเมิน",...ODI_SECTIONS.map(s=>s.titleTH)];
      const rows = records.map((r,i) => [i+1, r.patient.name, r.patient.hn||"-", r.patient.birthdate, cAge(r.patient.birthdate), r.patient.gender, r.percentage, r.totalScore, r.maxScore, r.levelTH, r.level, new Date(r.submittedAt).toLocaleString("th-TH"), ...ODI_SECTIONS.map(s => (r.skippedSections||[]).includes(s.id) ? "ข้าม" : (r.answers?.[s.id] ?? "-"))]);
  ...
  const delRec=async(id)=>{if(!confirm("ต้องการลบข้อมูลนี้?"))return;const u=records.filter(r=>r.id!==id);await svR(u);sRec(u);if(sel?.id===id)sSel(null);};
  const clrAll=async()=>{if(!confirm("ต้องการลบข้อมูลทั้งหมด?"))return;await svR([]);sRec([]);sSel(null);};
  const saveUser=async()=>{if(!uf.username||!uf.displayName||(!editU&&!uf.password))return;let up=[...users];if(editU){up=up.map(u=>u.id===editU.id?{...u,...uf,password:uf.password||u.password}:u);}else{if(users.find(u=>u.username===uf.username)){alert("ชื่อผู้ใช้นี้มีอยู่แล้ว");return;}up.push({id:Date.now().toString(36),...uf,createdAt:new Date().toISOString()});}await svU(up);sUs(up);sSUF(false);sEU(null);sUF({username:"",password:"",displayName:"",role:"user"});};
  const delUser=async(id)=>{if(id==="admin"){alert("ไม่สามารถลบผู้ดูแลระบบหลักได้");return;}if(!confirm("ต้องการลบผู้ใช้นี้?"))return;const u=users.filter(x=>x.id!==id);await svU(u);sUs(u);};
